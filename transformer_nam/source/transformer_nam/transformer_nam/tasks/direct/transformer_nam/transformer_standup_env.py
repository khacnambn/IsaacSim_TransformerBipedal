"""
Transformer bipedal STAND UP — FULL 10 DOF (Bimo-style environment)
====================================================================
...
@configclass
class TransformerStandUpEnvCfg(DirectRLEnvCfg):
    ...
    weights = {
        "standup": [3.0, 2.0, 1.5, 0.0, 0.0, 1.0, 1.0],
    }
...
class TransformerStandUpEnv(DirectRLEnv):
    """Direct RL environment for Transformer (10 DOF, full leg control)"""

    cfg: TransformerStandUpEnvCfg

    def __init__(self, cfg: TransformerStandUpEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # StandUp requires wide range for BUB (split to stand)
        self.servo_max = torch.tensor(
            [ 110, 110,  30, 30,  20, 20,   5,  5,  30, 30],
        ...
        # Initial pose is SPLIT (BUB ~90 deg)
        start_pos = [91.7, 91.7, 0, 0, 0, 0, 0, 0, 0, 0]
        self.target_pos = torch.tensor([[0.0]*10 for _ in range(self.num_envs)], device=self.device)
        ...
        print(f"  Observation dim: {self.cfg.num_observations} (20 IMU + 40 action history)")
        print(f"  Action dim: {self.cfg.num_actions}")
        print(f"  Joints: Bub_L,Bub_R, Hip_L,Hip_R, Twist_L,Twist_R, Knee_L,Knee_R, Foot_L,Foot_R")
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        position_rew = joint_position_reward(self.cmd_actions, self.target_pos, self.device)
        sig_extra = torch.zeros_like(height_rew) # Not used
        vel_rew = velocity_penalty(lin_vel, self.robot.data.root_com_vel_w)
        feet_h_rew = torch.zeros_like(height_rew) # Not used
...
def joint_position_reward(pos_buff, start_pos, device: str):
    ...
    # Allow wide range for BUB
    max_diff = torch.tensor([110, 110, 30, 30, 30, 30, 70, 70, 45, 45], device=device)
...
@torch.jit.script
def velocity_penalty(lin_vel, full_vel):
    """Penalize all movement since the goal is to stand in place"""
    lin_norm = torch.norm(full_vel[:, :3], dim=1)
    ang_norm = torch.norm(full_vel[:, 3:], dim=1)
    return torch.exp(-2.0 * lin_norm) * torch.exp(-0.5 * ang_norm)
