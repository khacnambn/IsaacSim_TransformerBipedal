...
class TransformerTwistMarchEnv(DirectRLEnv):
    ...
    def _apply_action(self):
        ...
        if mask.any():
            ...
            cur     = as_torch(self.robot.data.joint_pos_target).clone()
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        root_pos = as_torch(self.robot.data.root_pos_w)              # (N,3)
        height   = root_pos[:, 2]                          # (N,)
        air_time = as_torch(self.scene.sensors["contact"].data.current_air_time)   # (N,2)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        height = as_torch(self.robot.data.root_pos_w)[:, 2]
        euler  = _quat_to_euler(as_torch(self.robot.data.root_quat_w))
        fallen = (
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        root_state = as_torch(self.robot.data.default_root_state)[env_ids].clone()
