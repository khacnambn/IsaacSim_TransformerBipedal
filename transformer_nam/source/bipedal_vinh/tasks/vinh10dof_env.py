"""
Transformer bipedal walking — FULL 10 DOF (Bimo-style environment)
====================================================================
Bản mở rộng của transformer_nam_env.py (6 DOF) sang đầy đủ 10 DOF trên
asset Fulltrans10DOF.usd. Thêm 2 khớp mới so với bản 6DOF:
  - Hip (lateral)  : RL tự học cân bằng ngang thay vì PD cứng bên ngoài
  - Twist          : RL tự học xoay hông (dự phòng cho rẽ hướng)

Joint order (10D, type-major / L-R minor — cùng convention với bản 6DOF):
  [Bub_L, Bub_R, Hip_L, Hip_R, Twist_L, Twist_R, Knee_L, Knee_R, Foot_L, Foot_R]

Episode bắt đầu từ tư thế ĐỨNG (không phải xoạc) — xem
transformer_config_10dof.py::TRANSFORMER_10DOF_CFG.init_state.

Observation (60D) — CÙNG cấu trúc với bản 6DOF, chỉ đổi action-history dim:
  [0:20]  IMU history: 4 timestep × (roll, pitch, gx, gy, gz)
  [20:60] action history: 4 timestep × 10 khớp (cmd_actions đã normalize)
"""
...
@configclass
class TransformerWalk10DOFEnvCfg(DirectRLEnvCfg):
    """Config for Transformer walking (10 DOF - full leg control)"""

    episode_length_s = 10.0
    decimation = 10
    num_actions = 10  # Bub, Hip(lat), Twist, Knee, Foot × 2 chân
    num_observations = 60  # 20 IMU history + 40 action history (4 x 10 joints)
    num_states = 0

    observation_space = gym.spaces.Box(
        low=-float("inf"), high=float("inf"), shape=(60,), dtype=float
    )
    ...
    action_space = gym.spaces.Box(low=-3.0, high=3.0, shape=(10,), dtype=float)

    obj = "walk"

    # 7 trọng số reward — GIỮ NGUYÊN cấu trúc từ bản 6DOF (không phụ thuộc
    # số khớp): [orientation, height, position, sig_extra, feet_h, velocity, deviation]
    weights = {
    ...
    sim: SimulationCfg = SimulationCfg(
        ...
        # Buffer PhysX cắt cho vừa bài toán này. Mặc định IsaacLab sized cho
        # manipulation hàng nghìn điểm tiếp xúc; robot đây chỉ chạm đất ở Foot.*
        # nên phần lớn buffer là cấp phát thừa — mà chúng nằm ở pinned host
        # memory, loại KHÔNG swap được, nên trên máy 16GB nó chặn num_envs
        # trước cả khi VRAM đầy (đo được: 8192 env -> RAM còn 2.9Gi, bắt đầu swap).
        physics=PhysxCfg(
            # Không có soft body / particle nào trong scene -> hạ sát đáy.
            gpu_max_soft_body_contacts=2**10,  # mặc định 2**20
            gpu_max_particle_contacts=2**10,  # mặc định 2**20
            # Tiếp xúc rigid: chỉ 2 bàn chân chạm đất mỗi env.
            gpu_max_rigid_contact_count=2**20,  # mặc định 2**23
            gpu_collision_stack_size=2**24,  # mặc định 2**26
            gpu_temp_buffer_capacity=2**23,  # mặc định 2**24
            # Aggregate pairs: scene chỉ có robot + mặt đất, không có vật thể rời.
            gpu_found_lost_aggregate_pairs_capacity=2**22,  # mặc định 2**25
        ),
    )
...
class TransformerWalk10DOFEnv(DirectRLEnv):
    """Direct RL environment for Transformer (10 DOF, full leg control)"""

    cfg: TransformerWalk10DOFEnvCfg

    def __init__(
        self, cfg: TransformerWalk10DOFEnvCfg, render_mode: str | None = None, **kwargs
    ):
        super().__init__(cfg, render_mode, **kwargs)

        self.weights = torch.tensor(
            self.cfg.weights[self.cfg.obj], device=self.device
        ).repeat(self.num_envs, 1)
        self.obj = self.cfg.obj

        # [Bub_L, Bub_R, Hip_L, Hip_R, Twist_L, Twist_R, Knee_L, Knee_R, Foot_L, Foot_R]
        # SỬA: base_pose trước đây đặt 25° vào Bub (nhầm — dùng nhầm giá trị
        # crouch cũ của bản 6DOF, lúc đó CHƯA xác định được Bub≠Hip thật).
        # Mục tiêu hiện tại: đứng THẲNG CHÂN (tất cả = 0°), biên độ RỘNG quanh
        # 0° để đủ tạo dáng đi thật — không dùng ±5° hẹp như crouch nữa.
        #   Bub (frontal/roll, URDF ±14.3~179.8°): biên hẹp, chỉ hỗ trợ cân bằng
        #   Hip (sagittal/đi bộ, URDF ±37.3°): biên rộng — khớp chính tạo dáng đi
        #   Twist (yaw, URDF ±180°): biên vừa, dự phòng rẽ hướng
        #   Knee (URDF ±126°, chỉ gập 1 chiều âm): biên rộng phía âm
        #   Foot (URDF -65.2°~40.3°): biên rộng
        self.servo_max = torch.tensor(
            [15, 15, 30, 30, 20, 20, 5, 5, 30, 30], device=self.device, dtype=torch.int
        )
        self.servo_min = torch.tensor(
            [-15, -15, -30, -30, -20, -20, -70, -70, -45, -45],
            device=self.device,
            dtype=torch.int,
        )
        ...
        self.last_direction = torch.zeros(self.num_envs, 10, device=self.device)
        ...
        self.frictions = torch.tensor(
            [0.3 + x / 1000 for x in range(0, 201)], device=self.device
        )
        self.torques = torch.tensor(
            [9.27 + x / 1000 for x in range(0, 1030)], device=self.device
        )
        self.dampings = torch.tensor(
            [0.6 + x / 1000 for x in range(0, 101)], device=self.device
        )
        ...
        self.act_hist = torch.zeros(self.num_envs, 4, 10, device=self.device)

        self.act_hist[:, :] = torch.clamp(
            (self.base_pose[0] - self.servo_min) / (self.servo_max - self.servo_min) * 2
            - 1,
            -1,
            1,
        )
        ...
        self._should_randomize_imu = (
            self.cfg.domain_rand if hasattr(self.cfg, "domain_rand") else False
        )

        print(f"\n{'='*70}")
        print(f"🤖 TRANSFORMER ENV (10 DOF - FULL LEG CONTROL)")
        print(
            f"  Observation dim: {self.cfg.num_observations} (20 IMU + 40 action history)"
        )
        print(f"  Action dim: {self.cfg.num_actions}")
        print(
            f"  Joints: Bub_L,Bub_R, Hip_L,Hip_R, Twist_L,Twist_R, Knee_L,Knee_R, Foot_L,Foot_R"
        )
        print(f"  Init pose: STANDING (0,0,0,0,0 deg) — not split")
    ...
    def _get_observations(self) -> dict:
        ...
        orient_noise = gaussian_noise(
            orient,
            GaussianNoiseCfg(mean=0.0, std=self.cfg.imu_noise_std["orientation"]),
        )
        gyro_noise_base = gaussian_noise(
            angular_vel_raw,
            GaussianNoiseCfg(mean=0.0, std=self.cfg.imu_noise_std["angular_velocity"]),
        )
        ...
        dt = 0.005
        self.gyro_drift += (
            self.cfg.imu_drift_rate * torch.randn_like(self.gyro_drift) * dt
        )
        ...
        cmd_act = torch.clamp(
            (self.cmd_actions - self.servo_min) / (self.servo_max - self.servo_min) * 2
            - 1,
            -1,
            1,
        )
        ...
        proc_act = self.act_hist.reshape(self.num_envs, 40)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        position_rew = joint_position_reward(
            self.cmd_actions, self.base_pose, self.device
        )
        ...
        if self.episode_length_buf[0] % 100 == 0 and self.episode_length_buf[0] > 0:
            ...
            print(f"\n{'='*70}")
            print(
                f"[Step {self.common_step_counter}] Episode {self.episode_length_buf[idx].item()}"
            )
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        fric_idx = torch.randint(
            0, self.frictions.size(0), (n_reset, n_joints), device=self.device
        )
        torque_idx = torch.randint(
            0, self.torques.size(0), (n_reset, n_joints), device=self.device
        )
        damp_idx = torch.randint(
            0, self.dampings.size(0), (n_reset, n_joints), device=self.device
        )
        ...
        self.robot.write_joint_friction_coefficient_to_sim(
            fric_samples, joint_ids=None, env_ids=env_ids
        )
        self.robot.write_joint_effort_limit_to_sim(
            torque_samples, joint_ids=None, env_ids=env_ids
        )
        self.robot.write_joint_damping_to_sim(
            damp_samples, joint_ids=None, env_ids=env_ids
        )
...
# ============================================================
# REWARD FUNCTIONS (giống hệt bản 6DOF — không phụ thuộc số khớp,
# chỉ position_reward/sigmoid_extra nhận tensor 10D thay vì 6D)
# ============================================================
...
def deviation_reward(og_pose, curr_pose, action: str = "walk"):
    ...
    if action == "walk":
        reward = torch.where(
            y_dev <= 0.3, 1 - torch.sqrt(y_dev / 0.3), torch.ones_like(y_dev) * -1
        )
    else:
        dist = x_dev + y_dev
        reward = torch.where(
            dist <= 0.3, 1 - torch.sqrt(dist / 0.3), torch.ones_like(dist) * -1
        )
...
def height_reward(robot_root_pos):
    """Height reward — ideal_height = 0.43m, đo thực nghiệm PyBullet với robot
    đứng thẳng chân (Bub=Hip=Knee=Foot=0°), KHÁC 0.392 của bản crouch cũ.
    Verify lại sau khi chạy rollout đầu tiên nếu robot lún/nhón chân bất thường."""
...
def joint_position_reward(pos_buff, start_pos, device: str):
    """Joint position reward — 10 khớp. Dung sai 20° (rộng hơn 5° của bản
    crouch cũ) vì servo_min/max giờ rộng quanh 0° để đủ tạo dáng đi thật."""
    max_diff = torch.tensor([20, 20, 20, 20, 20, 20, 20, 20, 20, 20], device=device)
...
def feet_height_reward(air_time, feet_pos, target_h: float, scale: float = 25.0):
    ...
    reward_per_leg = (
        torch.where(
            z_pos >= target_h, torch.ones_like(z_pos), torch.exp(-scale * z_err)
        )
        * in_air.float()
    )
